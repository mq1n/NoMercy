...
namespace
{
	...
	NTSTATUS FASTCALL UserIOPing(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		if (RequestInfo->InputBufferSize != sizeof(SKernelIOPingData) || RequestInfo->OutputBufferSize != sizeof(SKernelIOPingData))
			return STATUS_INFO_LENGTH_MISMATCH;

		if (!RequestInfo->InputBuffer || !RequestInfo->OutputBuffer)
			return STATUS_INVALID_PARAMETER;

		static_cast<PKernelIOPingData>(RequestInfo->OutputBuffer)->iMessage = 1337;
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_READ_MSR_IN) || RequestInfo->OutputBufferSize != sizeof(KB_READ_MSR_OUT))
			return STATUS_INFO_LENGTH_MISMATCH;

		if (!RequestInfo->InputBuffer || !RequestInfo->OutputBuffer) 
			return STATUS_INVALID_PARAMETER;

		static_cast<PKB_READ_MSR_OUT>(RequestInfo->OutputBuffer)->Value = CPU::RDMSR(static_cast<PKB_READ_MSR_IN>(RequestInfo->InputBuffer)->Index);
	...
	NTSTATUS FASTCALL KbWriteMsr(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength) 
	{
		...
		CPU::WRMSR(static_cast<PKB_WRITE_MSR_IN>(RequestInfo->InputBuffer)->Index, static_cast<PKB_WRITE_MSR_IN>(RequestInfo->InputBuffer)->Value);
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_CPUID_IN) || RequestInfo->OutputBufferSize != sizeof(KB_CPUID_OUT))
			return STATUS_INFO_LENGTH_MISMATCH;

		if (!RequestInfo->InputBuffer || !RequestInfo->OutputBuffer)
			return STATUS_INVALID_PARAMETER;

		CPU::CPUID(static_cast<PKB_CPUID_IN>(RequestInfo->InputBuffer)->FunctionIdEax, static_cast<CPU::PCPUID_INFO>(RequestInfo->OutputBuffer));
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_CPUIDEX_IN) || RequestInfo->OutputBufferSize != sizeof(KB_CPUID_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;

		if (!RequestInfo->InputBuffer || !RequestInfo->OutputBuffer) 
			return STATUS_INVALID_PARAMETER;

		CPU::CPUIDEX(static_cast<PKB_CPUIDEX_IN>(RequestInfo->InputBuffer)->FunctionIdEax, static_cast<PKB_CPUIDEX_IN>(RequestInfo->InputBuffer)->SubfunctionIdEcx, 
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_READ_PMC_IN) || RequestInfo->OutputBufferSize != sizeof(KB_READ_PMC_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;

		if (!RequestInfo->InputBuffer || !RequestInfo->OutputBuffer) 
			return STATUS_INVALID_PARAMETER;

		static_cast<PKB_READ_PMC_OUT>(RequestInfo->OutputBuffer)->Value = CPU::RDPMC(static_cast<PKB_READ_PMC_IN>(RequestInfo->InputBuffer)->Counter);
	...
	NTSTATUS FASTCALL KbReadTscp(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength) 
	{
		...
		static_cast<PKB_READ_TSCP_OUT>(RequestInfo->OutputBuffer)->Value = CPU::RDTSCP(&static_cast<PKB_READ_TSCP_OUT>(RequestInfo->OutputBuffer)->TscAux);
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_ALLOC_KERNEL_MEMORY_IN) || RequestInfo->OutputBufferSize != sizeof(KB_ALLOC_KERNEL_MEMORY_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		Output->KernelAddress = 
			reinterpret_cast<WdkTypes::PVOID>(Input->Executable ? VirtualMemory::AllocFromPoolExecutable(Input->Size) : VirtualMemory::AllocFromPool(Input->Size));
	...
	NTSTATUS FASTCALL KbCopyMoveMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength) 
	{
		...
		if (!Input || !Input->Src || !Input->Dest)
			return STATUS_INVALID_PARAMETER;

		if (Input->Intersects)
			RtlMoveMemory(reinterpret_cast<PVOID>(Input->Dest), reinterpret_cast<PVOID>(Input->Src), Input->Size);
		else
			RtlCopyMemory(reinterpret_cast<PVOID>(Input->Dest), reinterpret_cast<PVOID>(Input->Src), Input->Size);
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_EQUAL_MEMORY_IN) || RequestInfo->OutputBufferSize != sizeof(KB_EQUAL_MEMORY_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		static_cast<PKB_EQUAL_MEMORY_OUT>(RequestInfo->OutputBuffer)->Equals = RtlEqualMemory(reinterpret_cast<PVOID>(Input->Src), reinterpret_cast<PVOID>(Input->Dest), Input->Size);
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_MAP_MDL_IN) || RequestInfo->OutputBufferSize != sizeof(KB_MAP_MDL_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		{ 
			SrcProcess = Processes::Descriptors::GetEPROCESS(reinterpret_cast<HANDLE>(Input->SrcProcessId));
			if (!SrcProcess) 
		...
		{ 
			DestProcess = Processes::Descriptors::GetEPROCESS(reinterpret_cast<HANDLE>(Input->DestProcessId));
			if (!DestProcess) 
			{
				if (SrcProcess)
		...
		PVOID Mapping = NULL;
		NTSTATUS Status = Mdl::MapMdl(reinterpret_cast<PMDL>(Input->Mdl), &Mapping, SrcProcess, DestProcess, Input->NeedLock, static_cast<KPROCESSOR_MODE>(Input->AccessMode),
			Input->Protect, static_cast<MEMORY_CACHING_TYPE>(Input->CacheType), reinterpret_cast<PVOID>(Input->UserRequestedAddress));
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_MAP_MEMORY_IN) || RequestInfo->OutputBufferSize != sizeof(KB_MAP_MEMORY_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		NTSTATUS Status = Mdl::MapMemory(&MappingInfo, SrcProcess, DestProcess, reinterpret_cast<PVOID>(Input->VirtualAddress), Input->Size,
			static_cast<KPROCESSOR_MODE>(Input->AccessMode), Input->Protect, static_cast<MEMORY_CACHING_TYPE>(Input->CacheType), 
	...
	NTSTATUS FASTCALL KbProtectMappedMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength) 
	{
		...
		return MmProtectMdlSystemAddress(reinterpret_cast<PMDL>(Input->Mdl), Input->Protect);
	}
	...
	NTSTATUS FASTCALL KbUnmapMdl(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		if (!Input || !Input->Mdl || !Input->BaseAddress) 
			return STATUS_INVALID_PARAMETER;
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_MAP_PHYSICAL_MEMORY_IN) || RequestInfo->OutputBufferSize != sizeof(KB_MAP_PHYSICAL_MEMORY_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		if (!Input || !Input->Size || !Output)
			return STATUS_INVALID_PARAMETER;

		Output->VirtualAddress = reinterpret_cast<WdkTypes::PVOID>(PhysicalMemory::MapPhysicalMemory(reinterpret_cast<PVOID64>(Input->PhysicalAddress), Input->Size));

		*ResponseLength = RequestInfo->OutputBufferSize;

		return Output->VirtualAddress ? STATUS_SUCCESS : STATUS_GENERIC_NOT_MAPPED;
	}
	...
	NTSTATUS FASTCALL KbUnmapPhysicalMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		auto Input = static_cast<PKB_MAP_PHYSICAL_MEMORY_IN>(RequestInfo->InputBuffer);
		if (!Input || !Input->PhysicalAddress || !Input->Size) 
			return STATUS_INVALID_PARAMETER;

		PhysicalMemory::UnmapPhysicalMemory(reinterpret_cast<PVOID64>(Input->PhysicalAddress), Input->Size);
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_GET_PHYSICAL_ADDRESS_IN) || RequestInfo->OutputBufferSize != sizeof(KB_GET_PHYSICAL_ADDRESS_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		if (!Input || !Output) 
         ...
         Output->PhysicalAddress = 
			 reinterpret_cast<WdkTypes::PVOID>(PhysicalMemory::GetPhysicalAddress(reinterpret_cast<PVOID>(Input->VirtualAddress), reinterpret_cast<PEPROCESS>(Input->Process)));
	...
	NTSTATUS FASTCALL KbReadPhysicalMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		return PhysicalMemory::ReadPhysicalMemory(reinterpret_cast<PVOID64>(Input->PhysicalAddress), reinterpret_cast<PVOID>(&Output->Buffer), RequestInfo->OutputBufferSize) 
			? STATUS_SUCCESS 
			: STATUS_UNSUCCESSFUL;
	}
	...
	NTSTATUS FASTCALL KbWritePhysicalMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		if (!Input || !Input->Buffer || !Input->Size) 
			return STATUS_INVALID_PARAMETER;

		return PhysicalMemory::WritePhysicalMemory(reinterpret_cast<PVOID64>(Input->PhysicalAddress), reinterpret_cast<PVOID>(Input->Buffer), Input->Size) 
			? STATUS_SUCCESS 
			: STATUS_UNSUCCESSFUL;
	}
	...
	NTSTATUS FASTCALL KbReadDmiMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		BOOLEAN Status = PhysicalMemory::ReadDmiMemory(reinterpret_cast<PVOID>(Output->DmiBuffer), DmiSize);
		if (!Status) 
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_GET_EPROCESS_IN) || RequestInfo->OutputBufferSize != sizeof(KB_GET_EPROCESS_OUT))
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		Output->Process = reinterpret_cast<WdkTypes::PEPROCESS>(Processes::Descriptors::GetEPROCESS(reinterpret_cast<HANDLE>(Input->ProcessId)));
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_GET_ETHREAD_IN) || RequestInfo->OutputBufferSize != sizeof(KB_GET_ETHREAD_OUT))
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		Output->Thread = reinterpret_cast<WdkTypes::PEPROCESS>(Processes::Descriptors::GetETHREAD(reinterpret_cast<HANDLE>(Input->ThreadId)));
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_OPEN_PROCESS_IN) || RequestInfo->OutputBufferSize != sizeof(KB_OPEN_PROCESS_OUT))
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		NTSTATUS Status = Processes::Descriptors::OpenProcess(reinterpret_cast<HANDLE>(Input->ProcessId), &hProcess, Input->Access, Input->Attributes);
		if (!NT_SUCCESS(Status)) 
			return Status;
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_OPEN_PROCESS_BY_POINTER_IN) || RequestInfo->OutputBufferSize != sizeof(KB_OPEN_PROCESS_OUT))
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		HANDLE hProcess = NULL;
		NTSTATUS Status = Processes::Descriptors::OpenProcessByPointer(reinterpret_cast<PEPROCESS>(Input->Process), &hProcess, Input->Access, Input->Attributes, 
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_OPEN_THREAD_IN) || RequestInfo->OutputBufferSize != sizeof(KB_OPEN_THREAD_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		HANDLE hThread = NULL;
		NTSTATUS Status = Processes::Descriptors::OpenThread(reinterpret_cast<HANDLE>(Input->ThreadId), &hThread, Input->Access, Input->Attributes);
		if (!NT_SUCCESS(Status)) 
			return Status;
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_OPEN_THREAD_BY_POINTER_IN) || RequestInfo->OutputBufferSize != sizeof(KB_OPEN_THREAD_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		HANDLE hThread = NULL;
		NTSTATUS Status = Processes::Descriptors::OpenThreadByPointer(reinterpret_cast<PETHREAD>(Input->Thread), &hThread, Input->Access, Input->Attributes, 
			static_cast<KPROCESSOR_MODE>(Input->ProcessorMode));
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_ALLOC_USER_MEMORY_IN) || RequestInfo->OutputBufferSize != sizeof(KB_ALLOC_USER_MEMORY_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		NTSTATUS Status = Input->ProcessId ? Processes::Descriptors::OpenProcess(reinterpret_cast<HANDLE>(Input->ProcessId), &hProcess) : STATUS_SUCCESS;
		if (!NT_SUCCESS(Status)) 
			return STATUS_UNSUCCESSFUL;

		PVOID BaseAddress = NULL;
		Status = Processes::MemoryManagement::AllocateVirtualMemory(hProcess, Input->Size, Input->Protect, &BaseAddress);
	...
	NTSTATUS FASTCALL KbFreeUserMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		Status = Processes::MemoryManagement::FreeVirtualMemory(hProcess, reinterpret_cast<PVOID>(Input->BaseAddress));
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_SECURE_VIRTUAL_MEMORY_IN) || RequestInfo->OutputBufferSize != sizeof(KB_SECURE_VIRTUAL_MEMORY_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		{
			Status = VirtualMemory::SecureMemory(reinterpret_cast<PVOID>(Input->BaseAddress), Input->Size, Input->ProtectRights, &SecureHandle);
		} 
		else 
		{
			...
			Status = VirtualMemory::SecureProcessMemory(Process, reinterpret_cast<PVOID>(Input->BaseAddress), Input->Size, Input->ProtectRights, &SecureHandle);

			ObDereferenceObject(Process);
		}

		if (!Status || !SecureHandle) 
			return STATUS_UNSUCCESSFUL;
	...
	NTSTATUS FASTCALL KbUnsecureVirtualMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		if (!Input || !Input->ProcessId || !Input->SecureHandle)
			return STATUS_INVALID_PARAMETER;
		...
		else 
		{
			...
			VirtualMemory::UnsecureProcessMemory(Process, reinterpret_cast<HANDLE>(Input->SecureHandle));
			ObDereferenceObject(Process);
		}
	...
	NTSTATUS FASTCALL KbReadProcessMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		NTSTATUS Status = Processes::MemoryManagement::ReadProcessMemory(Process, reinterpret_cast<PVOID>(Input->BaseAddress), reinterpret_cast<PVOID>(Input->Buffer),
			Input->Size);
	...
	NTSTATUS FASTCALL KbWriteProcessMemory(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		NTSTATUS Status = Processes::MemoryManagement::WriteProcessMemory(Process, reinterpret_cast<PVOID>(Input->BaseAddress), reinterpret_cast<PVOID>(Input->Buffer), 
			Input->Size);
	...
	NTSTATUS FASTCALL KbGetThreadContext(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		if (Input->ContextSize != sizeof(CONTEXT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_CREATE_USER_THREAD_IN) || RequestInfo->OutputBufferSize != sizeof(KB_CREATE_USER_SYSTEM_THREAD_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		HANDLE hProcess;
		NTSTATUS Status = Processes::Descriptors::OpenProcess(reinterpret_cast<HANDLE>(Input->ProcessId), &hProcess);
		if (!NT_SUCCESS(Status)) 
			return STATUS_NOT_FOUND;

		HANDLE hThread = NULL;
		CLIENT_ID ClientId = {};
		Status = Processes::Threads::CreateUserThread(hProcess, reinterpret_cast<Processes::Threads::_UserThreadRoutine>(Input->ThreadRoutine), 
			reinterpret_cast<PVOID>(Input->Argument), Input->CreateSuspended, &hThread, &ClientId);
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_CREATE_SYSTEM_THREAD_IN) || RequestInfo->OutputBufferSize != sizeof(KB_CREATE_USER_SYSTEM_THREAD_OUT))
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		if (Input->AssociatedProcessId && Input->AssociatedProcessId != 4) 
		{
			...
			Status = Processes::Descriptors::OpenProcess(reinterpret_cast<HANDLE>(Input->AssociatedProcessId), &hProcess);
			if (!NT_SUCCESS(Status))
				return STATUS_NOT_FOUND;
		}
	...
	NTSTATUS FASTCALL KbQueueUserApc(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		NTSTATUS Status = Processes::Apc::QueueUserApc(Thread, reinterpret_cast<Processes::Apc::PKNORMAL_ROUTINE>(Input->ApcProc), reinterpret_cast<PVOID>(Input->Argument));
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_EXECUTE_SHELL_CODE_IN) || RequestInfo->OutputBufferSize != sizeof(KB_EXECUTE_SHELL_CODE_OUT)) 
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		Output->Result = KernelShells::ExecuteShellCode(reinterpret_cast<KernelShells::_ShellCode>(Input->Address),reinterpret_cast<PVOID>(Input->Argument));
	...
	{
		if (RequestInfo->InputBufferSize != sizeof(KB_GET_KERNEL_PROC_ADDRESS_IN) || RequestInfo->OutputBufferSize != sizeof(KB_GET_KERNEL_PROC_ADDRESS_OUT))
			return STATUS_INFO_LENGTH_MISMATCH;
		...
		HANDLE hSecure = NULL;
		BOOLEAN SecureStatus = VirtualMemory::SecureMemory(reinterpret_cast<PVOID>(Input->RoutineName), Input->SizeOfBufferInBytes, PAGE_READONLY, &hSecure);

		if (!SecureStatus) 
			return STATUS_UNSUCCESSFUL;

		LPWSTR RoutineNameKernelBuffer = VirtualMemory::AllocWideString(Input->SizeOfBufferInBytes / sizeof(WCHAR));
	...
	NTSTATUS FASTCALL KbCreateDriver(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
	{
		...
		using _DriverEntry = NTSTATUS(NTAPI*)(_In_ PDRIVER_OBJECT DriverObject, _In_ PUNICODE_STRING RegistryPath);

		HANDLE hSecure = NULL;
		BOOLEAN SecureStatus = VirtualMemory::SecureMemory(reinterpret_cast<PVOID>(Input->DriverName), Input->DriverNameSizeInBytes, PAGE_READONLY, &hSecure);
		if (!SecureStatus) 
			return STATUS_UNSUCCESSFUL;

		LPWSTR DriverNameKernelBuffer = VirtualMemory::AllocWideString(Input->DriverNameSizeInBytes / sizeof(WCHAR));
...
NTSTATUS FASTCALL DispatchIOCTL(IN PIOCTL_INFO RequestInfo, OUT PSIZE_T ResponseLength)
{
	...
	if (Index < sizeof(Handlers) / sizeof(Handlers[0]))
		return Handlers[Index](RequestInfo, ResponseLength);
