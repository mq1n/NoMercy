	...
	public:
		...
		class ListIterator 
		{
			public:
				...
				ListIterator(PLIST_ENTRY Head, ListEntry* Entry) : 
					ListHead(Head), Current(Entry->GetEntry())
				{
				}
				...
				ListEntry* GetEntry() 
				{ 
					return static_cast<ListEntry*>(Current->EntryObject); 
				}

				ListIterator& operator ++ () 
				{
					Current = Current->ChainEntry.Flink != ListHead ? reinterpret_cast<_Entry*>(Current->ChainEntry.Flink) : NULL;
					return *this;
				}

				ListIterator& operator -- () 
				{
					Current = Current->ChainEntry.Flink != ListHead ? reinterpret_cast<_Entry*>(Current->ChainEntry.Blink) : NULL;
					return *this;
				}
				...
				{
					return !Iterator.Current || !Current ? Iterator.Current == Current : Iterator.Current->EntryObject == Current->EntryObject;
				}
