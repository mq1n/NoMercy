...
ObCallbacks::ObCallbacks(OPTIONAL POB_PRE_OPERATION_CALLBACK PreCallback, OPTIONAL POB_POST_OPERATION_CALLBACK PostCallback, OPTIONAL PVOID RegistrationContext, 
	ObCallbackType CallbackType, OB_OPERATION OperationType) :
	ObCallbacks() 
{
...
NTSTATUS ObCallbacks::SetupCallbacks(OPTIONAL POB_PRE_OPERATION_CALLBACK PreCallback, OPTIONAL POB_POST_OPERATION_CALLBACK PostCallback, OPTIONAL PVOID RegistrationContext, 
	ObCallbackType CallbackType, OB_OPERATION OperationType)
{
	...
	Registration.OperationRegistrationCount = CallbackType == ctAll ? sizeof(Operations) / sizeof(*Operations) : 1;
	Registration.RegistrationContext = RegistrationContext;
	Registration.OperationRegistration = 
		CallbackType == ctAll ? reinterpret_cast<POB_OPERATION_REGISTRATION>(&Operations) : reinterpret_cast<POB_OPERATION_REGISTRATION>(&Operations[CallbackType - 1]);
