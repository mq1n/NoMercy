...
*/
	...
	PVOID AllocFromPool(SIZE_T Bytes, BOOLEAN FillByZeroes) 
	{
		...
		VOID* CONST Address = ExAllocatePoolWithTag(ExDefaultNonPagedPoolType,Bytes, PoolTag);
		if (!Address) 
			return NULL;
	...
	PVOID AllocFromPoolExecutable(SIZE_T Bytes) 
	{
		...
		return ExAllocatePoolWithTag(NonPagedPoolExecute, Bytes, PoolTag);
	}
	...
	_IRQL_requires_max_(APC_LEVEL)
	BOOLEAN SecureMemory(__in_data_source(USER_MODE) PVOID UserAddress, SIZE_T Size, ULONG ProtectRights, OUT PHANDLE SecureHandle)
	{
	...
	_IRQL_requires_max_(APC_LEVEL)
	BOOLEAN SecureProcessMemory(PEPROCESS Process,__in_data_source(USER_MODE) PVOID UserAddress, SIZE_T Size, ULONG ProtectRights, OUT PHANDLE SecureHandle)
	{
		if (!Process || !SecureHandle)
			return FALSE;
...
namespace Heap 
{
	...
	PVOID CreateHeap() 
	{
		return RtlCreateHeap(HEAP_GROWABLE, NULL, PAGE_SIZE * 2, PAGE_SIZE, NULL, NULL);
	}
...
namespace PhysicalMemory 
{
	...
	{
		return MmAllocateContiguousMemory(Size, *reinterpret_cast<PHYSICAL_ADDRESS*>(HighestAcceptableAddress));
	}
	...
	{
		return MmIsAddressValid(VirtualAddress) ? reinterpret_cast<PVOID64>(MmGetPhysicalAddress(VirtualAddress).QuadPart) : NULL;
	}
		...
		PVOID64 GetPhysicalAddress(PVOID VirtualAddress, OPTIONAL PEPROCESS Process) 
	{
		if (!Process || Process == PsGetCurrentProcess())
		{
			if (MmIsAddressValid(VirtualAddress))
				return reinterpret_cast<PVOID64>(MmGetPhysicalAddress(VirtualAddress).QuadPart);
			return NULL;
		}
		...
		PVOID64 PhysicalAddress = MmIsAddressValid(VirtualAddress) ? reinterpret_cast<PVOID64>(MmGetPhysicalAddress(VirtualAddress).QuadPart) : NULL;
...
{
	_IRQL_requires_max_(APC_LEVEL)
	PMDL AllocMdlAndLockPages(PVOID Address, ULONG Size, KPROCESSOR_MODE AccessMode, LOCK_OPERATION Operation, OPTIONAL PEPROCESS Process)
	{
		if (!Address || !Size) 
			return NULL;
	...
	_IRQL_requires_max_(APC_LEVEL)
	NTSTATUS MapMdl(IN PMDL Mdl, OUT PVOID* MappedMemory, OPTIONAL PEPROCESS SrcProcess, OPTIONAL PEPROCESS DestProcess, BOOLEAN NeedLock, KPROCESSOR_MODE AccessMode, 
		ULONG Protect, MEMORY_CACHING_TYPE CacheType, OPTIONAL PVOID UserRequestedAddress)
	{
		if (!MappedMemory)
		...
		*MappedMemory = NULL;
		...
		{
			if ((AccessMode == KernelMode && AddressRange::IsUserAddress(UserRequestedAddress)) ||
		...
		__try 
		{
			...
			PVOID Mapping = MmMapLockedPagesSpecifyCache(Mdl, AccessMode, CacheType, AccessMode == UserMode ? UserRequestedAddress : NULL, FALSE, NormalPagePriority);
	...
	NTSTATUS MapMemory(OUT PMAPPING_INFO MappingInfo, OPTIONAL PEPROCESS SrcProcess, OPTIONAL PEPROCESS DestProcess, IN PVOID VirtualAddress, ULONG Size, 
		KPROCESSOR_MODE AccessMode, ULONG Protect, MEMORY_CACHING_TYPE CacheType, OPTIONAL PVOID UserRequestedAddress ) 
	{
		if (!Size || !MappingInfo) 
			return STATUS_INVALID_PARAMETER;
		...
		*MappingInfo = {};
		...
		NTSTATUS Status = MapMdl(MappingInfo->Mdl, &MappingInfo->BaseAddress, SrcProcess, DestProcess, TRUE, AccessMode, Protect, CacheType, UserRequestedAddress);	
