...
namespace Processes 
{
	namespace Descriptors 
	{
		...
		NTSTATUS OpenProcess(HANDLE ProcessId, OUT PHANDLE hProcess, ACCESS_MASK AccessMask, ULONG Attributes)
		{
			CLIENT_ID ClientId;
		...
		_IRQL_requires_max_(PASSIVE_LEVEL)
		NTSTATUS OpenProcessByPointer(PEPROCESS Process, OUT PHANDLE hProcess, ACCESS_MASK AccessMask, ULONG Attributes, KPROCESSOR_MODE ProcessorMode)
		{
			return ObOpenObjectByPointer(Process, Attributes, NULL, AccessMask, *PsProcessType, ProcessorMode, hProcess);
		}

		_IRQL_requires_max_(PASSIVE_LEVEL)
		NTSTATUS OpenThread(HANDLE ThreadId, OUT PHANDLE hThread, ACCESS_MASK AccessMask, ULONG Attributes)
		{
			CLIENT_ID ClientId;
			...
			using _ZwOpenThread = NTSTATUS(NTAPI*)(OUT PHANDLE hThread, IN ACCESS_MASK AccessMask, IN POBJECT_ATTRIBUTES ObjectAttributes, IN PCLIENT_ID ClientId);
		...
		_IRQL_requires_max_(PASSIVE_LEVEL)
		NTSTATUS OpenThreadByPointer(PETHREAD Thread, OUT PHANDLE hThread, ACCESS_MASK AccessMask,  ULONG Attributes, KPROCESSOR_MODE ProcessorMode)
		{
			return ObOpenObjectByPointer(Thread, Attributes, NULL, AccessMask, *PsThreadType, ProcessorMode, hThread);
		}
	}
	...
	namespace Threads 
	{
		...
		{
			using _PsGetContextThread = NTSTATUS (NTAPI*)(IN PETHREAD Thread, OUT PCONTEXT Context, IN KPROCESSOR_MODE PreviousMode);

			static auto _GetContextThread = static_cast<_PsGetContextThread>(Importer::GetKernelProcAddress(L"PsGetContextThread"));
			if (_GetContextThread)
				return _GetContextThread(Thread, Context, PreviousMode);
		...
		{
			using _PsSetContextThread = NTSTATUS (NTAPI*)(IN PETHREAD Thread, IN PCONTEXT Context, IN KPROCESSOR_MODE PreviousMode);

			static auto _SetContextThread = static_cast<_PsSetContextThread>(Importer::GetKernelProcAddress(L"PsSetContextThread"));
			if (_SetContextThread)
				return SetContextThread(Thread, Context, PreviousMode);
		...
		{
			using _PsSuspendProcess = NTSTATUS (NTAPI*)(IN PEPROCESS Process);

			static auto _SuspendProcess = static_cast<_PsSuspendProcess>(Importer::GetKernelProcAddress(L"PsSuspendProcess"));
			if (_SuspendProcess)
				return _SuspendProcess(Process);
		...
		{
			using _PsResumeProcess = NTSTATUS (NTAPI*)(IN PEPROCESS Process);

			static auto _SuspendProcess = static_cast<_PsResumeProcess>(Importer::GetKernelProcAddress(L"PsResumeProcess"));
			if (_SuspendProcess)
				return _SuspendProcess(Process);
		...
		{
			using _RtlCreateUserThread = NTSTATUS (NTAPI*)(IN HANDLE ProcessHandle, IN PSECURITY_DESCRIPTOR SecurityDescriptor, IN BOOLEAN CreateSuspended,
				IN ULONG StackZeroBits, IN OUT PULONG StackReserved, IN OUT PULONG StackCommit, IN PVOID StartAddress, IN PVOID StartParameter, OUT PHANDLE ThreadHandle,
				OUT PCLIENT_ID ClientID
			);

			static auto _CreateUserThread = static_cast<_RtlCreateUserThread>(Importer::GetKernelProcAddress(L"RtlCreateUserThread"));
			if (_CreateUserThread)
				return _CreateUserThread(hProcess, NULL, CreateSuspended, 0, NULL, NULL, StartAddress, Argument, hThread, ClientId);
		...
		{
			using _ZwQueryInformationThread = NTSTATUS (NTAPI*)(HANDLE hThread, THREADINFOCLASS ThreadInformationClass, OUT PVOID ThreadInformation, ULONG ThreadInformationLength, OUT PULONG ReturnLength);

			static auto _QueryInformationThread = static_cast<_ZwQueryInformationThread>(Importer::GetKernelProcAddress(L"ZwQueryInformationThread"));
			if (_QueryInformationThread)
				return _QueryInformationThread(hThread, ThreadInformationClass, ThreadInformation, ThreadInformationLength, ReturnLength);
		...
		{
			return ZwSetInformationThread(hThread, ThreadInformationClass, ThreadInformation, ThreadInformationLength);
		}
	}
	...
	namespace Information 
	{
		...
		{
			using _ZwQueryInformationProcess = NTSTATUS (NTAPI*)(HANDLE hProcess, PROCESSINFOCLASS ProcessInformationClass, IN PVOID ProcessInformation, ULONG ProcessInformationLength, OUT PULONG ReturnLength);

			static auto _QueryInformationProcess = static_cast<_ZwQueryInformationProcess>(Importer::GetKernelProcAddress(L"ZwQueryInformationProcess"));
			if (_QueryInformationProcess)
				_QueryInformationProcess(hProcess, ProcessInformationClass, ProcessInformation, ProcessInformationLength, ReturnLength);
		...
		{
			using _ZwSetInformationProcess = NTSTATUS (NTAPI*)(HANDLE hProcess, PROCESSINFOCLASS ProcessInformationClass, IN PVOID ProcessInformation, ULONG ProcessInformationLength);

			static auto _SetInformationProcess = static_cast<_ZwSetInformationProcess>(Importer::GetKernelProcAddress(L"ZwSetInformationProcess"));
			if (_SetInformationProcess)
				return _SetInformationProcess(hProcess, ProcessInformationClass, ProcessInformation, ProcessInformationLength);
	...
	namespace MemoryManagement 
	{
		...
		PVOID AllocateVirtualMemory(HANDLE hProcess, SIZE_T Size, ULONG Protect) 
		{
			...
			return NT_SUCCESS(ZwAllocateVirtualMemory(hProcess, &BaseAddress, 0, &Size, MEM_COMMIT, Protect)) ? BaseAddress : NULL;
		}
		...
		_IRQL_requires_max_(APC_LEVEL)
		NTSTATUS OperateProcessMemory(PEPROCESS Process, __in_data_source(USER_MODE) PVOID BaseAddress, PVOID Buffer, ULONG Size, MEMORY_OPERATION_TYPE Operation)
		{
			if (!Process)
				return STATUS_INVALID_PARAMETER_1;

			if (!BaseAddress || AddressRange::IsKernelAddress(BaseAddress)) 
			...
			NTSTATUS Status = Mdl::MapMemory(&ProcessMapping, Process, NULL, BaseAddress, Size);
			...
			{ 
				...
				if (IsBufferUsermode) 
					VirtualMemory::UnsecureMemory(hBufferSecure);
			...
			Status = Mdl::MapMemory(&BufferMapping, NULL, NULL, Buffer, Size);
			if (!NT_SUCCESS(Status)) 
			{
	...
	namespace Apc 
	{
		...
		NTSTATUS QueueUserApc(PETHREAD Thread, PKNORMAL_ROUTINE NormalRoutine, PVOID Argument) 
		{
			...
			KeInitializeApc(UserApc, Thread, OriginalApcEnvironment,
				[](PRKAPC Apc, PKNORMAL_ROUTINE NormalRoutine, PVOID NormalContext, PVOID SystemArgument1, PVOID SystemArgument2) -> VOID 
				{
