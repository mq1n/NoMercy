...
class AnsiString : public String <CHAR> 
{
	...
	public:
		AnsiString(PCANSI_STRING Ansi) : 
			String(Ansi->Buffer, Ansi->Length / sizeof(CHAR)) 
		{
		}
		AnsiString(const String& Str) :
			String(Str) 
		{
		}
...
class WideString : public String <WCHAR> 
{
	...
	public:
		WideString(PCUNICODE_STRING Wide) :
			String(Wide->Buffer, Wide->Length / sizeof(WCHAR)) 
		{
		}
...
inline String <CHAR> FormatAnsi(LPCSTR Format, ...)
{
	...
	while ((characters = _vsnprintf_s(Result.GetData(), Result.GetLength() + 1, Result.GetLength(), Format, args)) == -1)
	{
...
inline String <WCHAR> FormatWide(LPCWSTR Format, ...)
{
	...
	while ((characters = _vsnwprintf_s(Result.GetData(), Result.GetLength() + 1, Result.GetLength(), Format, args)) == -1) 
	{
		Result.Resize(Result.GetLength() + BufferSize);
	}
	Result.Resize(characters);
